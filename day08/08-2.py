def main():
    ...
    while start or len(joins[0]) != 1000:
        start = False
        ...
        for from_point_canc in range(len(distances)):
            for index_cand, (to_point_cand, dist) in enumerate(
                distances[from_point_canc]
            ):
                if dist < min_dist:
                    min_dist = dist
        ...
        distances[from_point][index] = (to_point, float("inf"))
        last_connect = (from_point, to_point)
    ...
    x1, _, _ = points[last_connect[0]]
    x2, _, _ = points[last_connect[1]]
    print(x1 * x2)
